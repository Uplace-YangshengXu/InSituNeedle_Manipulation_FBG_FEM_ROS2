...
btz = 0; % needle base slope about z axis

x_pre = linspace(bx, bx + L, L + 1)'; % nodal x coordinates
y_pre = by*ones(size(x_pre)); % nodal y coordinates
z_pre = bz*ones(size(x_pre)); % nodal z coordinates
ty_pre = bty*ones(size(x_pre)); % nodal slopes about y axis
tz_pre = btz*ones(size(x_pre)); % nodal slopes about z axis
...
[x_new, y_new, z_new, ty_new, tz_new] = three_d_needle_FEM(L, Mu, Alpha, Interval, ...
          x_pre, y_pre, z_pre, ty_pre, tz_pre, ...
          0, 0, 0, 0, 0, ...
          curvatures_xz, curvatures_xy, AA_lcn);
% AA_lcn
        % update states
x_pre = x_new; 
y_pre = y_new; 
...
pub_msg.needle_z_axis = z_pre;
pub_msg.needle_x_axis = x_pre;
pub_msg.needle_y_axis = y_pre;
pub_msg.needle_slope  = ty_pre;
...
%% main loop
    while (1)
        ...
        ic = [x_pre(end);y_pre(end);z_pre(end);ty_pre(end);tz_pre(end);0;0;0;0;0];
        
        [dcontrol,desired]  = numerical_jacobian_3d_control(xd, Kp, ic, L, Mu, Alpha, Interval,...
                x_pre,y_pre,z_pre,ty_pre,tz_pre,...
                [],[],[],thre,desired);
%         disp(dcontrol)
%         disp(desired)
...
%         end
        ...
        ty_pre = ty_new; 
        tz_pre = tz_new;
        disp([x_pre(end) y_pre(end) z_pre(end) ty_pre(end) tz_pre(end)]);
        ...
        error = norm([x_new(end);y_new(end);z_new(end);ty_new(end);tz_new(end)] - xd(:,end));
        disp(error);
        toc
        % break critria
        if error <= 0.05

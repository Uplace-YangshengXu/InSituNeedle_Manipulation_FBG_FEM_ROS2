...
%% create publisher for shape comparision
if Compare_switch == 1

    if exist("publisher_fbg",'var')
        delete(publisher_fbg)
    end
    
    publisher_fbg = MatlabRosPubSub('pub','matlab_needle_shape_fbg_publisher','/needle_shape_fbg','fbg_msgs/NeedleShape');
    pub_msg_fbg = ros2message("fbg_msgs/NeedleShape"); % create message structure
    pub_msg_fbg.needle_total_length = uint8(L);
    pub_msg_fbg.active_area_location = AA_lcn;
    pub_msg_fbg.needle_z_axis = -20*ones(size(x_new));

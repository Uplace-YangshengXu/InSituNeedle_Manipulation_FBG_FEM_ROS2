...
motor_controller_ip = '192.168.1.201';%         [x_new, y_new, k_new] = planar_needle_FEM(L, Mu, Alpha, Interval, ...
%             x_pre, y_pre, k_pre, ...
%             dbx, dby, dbk, ...
%             curvatures_xz, AA_lcn);
motor_controller_port = 23;

%% FEM parameters and setup%         [x_new, y_new, k_new] = planar_needle_FEM(L, Mu, Alpha, Interval, ...
%             x_pre, y_pre, k_pre, ...
%             dbx, dby, dbk, ...
%             curvatures_xz, AA_lcn);
...
% AA_lcn = [44;79;114]; curvatures_xy
AA_lcn = [44;79]; 
...
if exist('client','var')
    ...
    curvatures_xy = - msg_received.curvature_xy; % rotary stage flipped
    curvatures_xz = - msg_received.curvature_xz;
end

[x_new, y_new, k_new,Constraints] = planar_needle_FEM_wc(L, Mu, Alpha, Interval, Ti,...
...
%% create publisher for nex_new, y_new, k_newedleshape
...
%         tic
        ...
        [dcontrol,desired] = numerical_jacobian_traj_following_control(xd, Kp, ic, L, Mu, Alpha, Interval, Ti,...
        x_pre,y_pre,k_pre,...
...
%         dbk = sign(dbk)*min(0.01,norm(dbk));
        ...
        Input_Rotation = Input_Rotation + round(-dr*7031.25); % rotary stage flipped
        % control motor
        if Motor_switch == 1
            % move motors
